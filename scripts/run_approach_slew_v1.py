"""Final conditional common-reference diagnostic; unchanged raw-force job rule."""
...
def main():
    ...
    approach_path = ROOT / "configs/approach_slew_registration_v1.json"
    approach = json.loads(approach_path.read_text())
    design = approach["design_evidence"]
    if sha256(ROOT / design["path"]) != design["sha256"]:
        raise ValueError("Geometric slew design evidence changed")
    if not args.plan:
        ...
        if (original.get("status") != "verified_diagnostic"
                ...
                or not (decision.get("finest_prefix_pair_pass") is False or any(
                    row["prefix_summary"]["active_at_prefix_end"] < 8 for row in original["runs"]))):
            raise ValueError("Final reference test requires verified servo sensitivity or inadequate active exposure")
    if args.plan:
        print(json.dumps({"refinements": args.refinements, "physical_registration": registration, "condition_registration": approach, "expected_charged_reference_transitions": approach["expected_charged_reference_transitions"]}, indent=2))
        return 0
    frozen = preflight()
    ...
    queue = {"schema": 1, "status": "running", "runs": [], "launcher_pid": os.getpid(),
             ...
             "approach_registration_sha256": sha256(approach_path)}
    ...
    for refinement in args.refinements:
        ...
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/probe_approach_slew_v1.py"),
        ...
        manifest = {"schema": 1, "status": "starting", "research_result": False,
                    ...
                    "approach_registration": approach, "approach_registration_sha256": sha256(approach_path),
        ...
        try:
            ...
            if manifest["status"] == "completed":
                from scripts.review_approach_slew_v1 import verify_single
