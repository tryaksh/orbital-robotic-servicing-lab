...
def main():
    ...
    registration = ROOT / "configs/failure_prefix_recovery_v1.json"
    ...
    queue = {"status": "running", "runs": [], "registration_sha256": sha256(registration),
             "expected_charged_transitions": 38503.5, "wall_limit_s": 1260}
    write_json(output / "queue.json", queue)
    deadline = time.monotonic() + queue["wall_limit_s"]
    for controller in ("learned", "retry", "continue"):
        run = output / controller
        run.mkdir(exist_ok=False)
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/diagnose_failure_prefix.py"), "--headless",
        ...
        if manifest["status"] != "completed":
            break
    queue["status"] = "completed" if len(queue["runs"]) == 3 and all(r["status"] == "completed" for r in queue["runs"]) else "failed_or_incomplete"
    write_json(output / "queue.json", queue)
