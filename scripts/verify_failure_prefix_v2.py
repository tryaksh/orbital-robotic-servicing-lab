"""Replay and verify the preregistered two-seed frozen-policy confirmation."""
...
def main():
    ...
    if args.output.exists() or (args.run / "verification").exists():
    ...
    for seed in registration["seeds"]:
        ...
        for arm in registration["controllers"]:
            ...
            local = {
                ...
                "unchanged_script_settings": report["controller_settings"] == json.loads((ROOT / "configs/retry_unload_realign_v1.json").read_text())
    ...
    raw_output = args.run / "verification"
