...
def main():
    ...
    protocol = ROOT / "configs/protocol_v2.json"
    ...
    checkpoint = ROOT / "artifacts/assembly/uniform_pilot_170_v2/pilot/budget_checkpoint.pt"
    registration = ROOT / "evidence/pilot_failure_diagnosis_v2.json"
    registered = json.loads(registration.read_text())
    if sha256(checkpoint) != registered["checkpoint_sha256"]:
        parser.error("Registered checkpoint mismatch")
    output.mkdir(parents=True, exist_ok=False)
    queue = {"status": "running", "runs": [], "registration_sha256": sha256(registration),
             "expected_charged_transitions": 38503.5, "wall_limit_s": 1800}
    write_json(output / "queue.json", queue)
    deadline = time.monotonic() + queue["wall_limit_s"]
    for controller in ("deterministic", "stochastic", "retry"):
        run = output / controller
        run.mkdir(exist_ok=False)
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/diagnose_policy.py"), "--headless",
        ...
        manifest = {"schema": 1, "status": "starting", "run_id": args.run_id + "/" + controller,
                    ...
                    "registration_sha256": sha256(registration), "environment_lock": json.loads(lock.read_text()),
                    ...
                    "scope": "Registered development diagnosis, frozen v2 task and controller checkpoint."}
