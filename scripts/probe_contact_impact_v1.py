"""Four-second frozen scripted-prefix impact diagnostic; no training or final tests."""
...
def main():
    ...
    parser.add_argument("--refinement", type=int, choices=(1, 2, 4, 8), required=True)
    ...
    report = {"status": "starting", "controller": args.controller, "seed": args.seed,
               "research_result": False, "reward_version": "completion_credit_v3", "diagnostic_protocol": "contact_impact_v1", "mode": args.mode, "refinement": args.refinement, "training_run_status": "completed", "checkpoint_selection": "Final registered 22-cohort budget checkpoint; no selection by score"}
    app = env = None
    try:
        ...
        env = PegContactImpactEnvV1(cfg, criteria=criteria, cases=cases, nominal=study["fault_support"]["nominal"], audit=True, refinement=args.refinement)
        ...
        report.update(
            ...
            scope_and_limitations=[
                ...
                "Native raw 20 N wrist abort, held-part gravity, retained grasp and existing success dwell remain unchanged. Finished jobs enter the existing commanded hold on the next 120 Hz servo tick.",
                "All processes initialize at 120 Hz. Explicit native dt changes only during live jobs; 15 Hz policy interface and 120 Hz servo/sensors, no extra RNG draws or within-job state writes.",
