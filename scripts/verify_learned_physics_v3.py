"""Verify fresh explicit-integration physics pairs and preserved reference compatibility."""
...
REGISTRATION = ROOT / "configs/learned_physics_validation_v3.json"
...
def verify_single_v1(path, registration):
    ...
    checks = {
        ...
        "explicit_native_integration_dt": bool((data["native"]["integration_dt"] == timing.physics_dt).all()) and report["scene_metadata_dt"] == 1 / 120,
...
def summarize_pairs_v1(root, registration):
    ...
    checks["queue_complete"] = queue["status"] == "completed" and len(queue["runs"]) == 6
    checks["queue_registration"] = queue["registration_sha256"] == sha256(REGISTRATION)
