...
def main():
    ...
    parser.add_argument("--controller", choices=("deterministic", "stochastic", "retry"), required=True)
    ...
    report = {"status": "starting", "controller": args.controller, "seed": 10070,
              "policy_rng_seed": 310070, "research_result": False}
    app = env = None
    try:
        ...
        names = ("actor_before", "mean_action", "requested_action", "applied_action", "target_xyz", "tool_xyz",
                 "part_relative_xyz", "part_quat", "raw_reward", "job_reward", "active", "dead_zone")
        ...
        for step in range(450):
            with torch.no_grad():
                ...
                values = (observation["policy"], mean, action, env.actions, target,
                          ...
                          extra["valid"], env.dead_zone_thresholds)
        ...
        report.update(jobs=jobs, recovery_witness=witness, discounted_job_returns=returns.cpu().tolist(),
                      ...
                      scope_and_limitations=["All 28 registered development cases; no optimization or final-test use.",
                          ...
                          "Returns omit partial terminal reward interval as frozen v2; original 0/84 result remains unchanged."])
