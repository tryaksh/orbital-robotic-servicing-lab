...
def main():
    ...
    registration = ROOT / "configs/partial_checkpoint_diagnosis_v2.json"
    ...
    failure = json.loads((ROOT / "evidence/uniform_completion_v3_r01_failure.json").read_text())
    if failure["status"] != "verified_failed_incomplete" or failure["completed_cohorts"] != 13:
        parser.error("Expected verified interrupted training record")
    if sha256(checkpoint) != failure["checkpoints"][-1]["sha256"]:
        parser.error("Last durable partial checkpoint mismatch")
    output.mkdir(parents=True, exist_ok=False)
    queue = {"status": "running", "runs": [], "registration_sha256": sha256(registration),
             "expected_charged_transitions": 51338.0, "wall_limit_s": 1680}
    ...
    for controller in ("deterministic", "stochastic", "clipped_mean", "retry"):
        ...
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/diagnose_partial_checkpoint.py"), "--headless",
        ...
        manifest = {"schema": 1, "status": "starting", "run_id": args.run_id + "/" + controller,
                    ...
                    "scope": "Partial-checkpoint diagnosis only; interrupted training remains failed. Four controllers, same physical task and v3 reward."}
