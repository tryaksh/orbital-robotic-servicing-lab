"""Registered uniform PPO with unclipped value loss and v3 completion credit; fresh training and budget-only evaluation."""
...
def main():
    ...
    report = {"status": "starting", "mode": args.mode, "seed": args.seed, "cohorts": [],
              "controller": "project_uniform_fault_ppo_value_unclipped_v4", "research_result": False, "reward_version": "completion_credit_v3", "optimizer_version": "value_unclipped_v4",
        ...
        for cohort in range(args.cohorts):
            ...
            if model is None:
                ...
                if training:
                    ...
                    if loaded["status"] != "budget_complete" or loaded["spec"] != model.spec or loaded.get("reward_version") != "completion_credit_v3" or loaded.get("optimizer_version") != "value_unclipped_v4":
            ...
            upstream_returns = torch.zeros_like(job_returns)
            credit_returns = torch.zeros_like(job_returns)
            ...
            item["upstream_discounted_job_returns"] = upstream_returns.cpu().tolist()
            item["completion_credit_discounted_returns"] = credit_returns.cpu().tolist()
            item["policy_action_std"] = model.log_std.clamp(-5, 2).exp().detach().cpu().tolist()
            ...
            diagnostic = {"case_indices": diagnostic_indices, "controls": torch.stack(diagnostic_rows).cpu(),
                          "columns": ["part_relative_xyz:3", "part_quaternion:4", "control_endpoint_raw_wrist_xyz:3",
                                      "applied_actions:7", "actor_observation_before:24", "raw_reward:1", "job_reward:1", "active_before:1", "completion_credit:1", "upstream_job_reward:1"]}
            if not training:
            ...
            if training:
                ...
                checkpoint = {"status": "budget_complete" if final else "partial", "model": model.state_dict(),
                              ...
                              "scope": "Fresh registered uniform development training. Resume equivalence is not verified."}
