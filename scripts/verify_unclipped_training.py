...
def credit_ledger_valid(cohorts):
    for cohort in cohorts:
        for job, total, upstream, credit in zip(cohort["jobs"], cohort["discounted_job_returns"],
                cohort["upstream_discounted_job_returns"], cohort["completion_credit_discounted_returns"], strict=True):
            native = round(job["elapsed_s"] * 120)
            included = native // 8
            expected = (35 / 12) * .995 ** included * (1 - .995 ** (450 - included)) / (1 - .995) if job["success"] else 0.
            if abs(credit - expected) > .002 or abs(total - upstream - credit) > .001:
                return False
    return True
...
def main():
    ...
    checks = {"training_process_complete": tm["status"] == tr["status"] == "completed",
              ...
              "eighty_eight_update_boundaries": len(tr["optimizer_updates"]) == 88,
              "all_training_requests_recorded": sum(len(c["jobs"]) for c in tr["cohorts"]) == 22528,
    ...
    checks["training_credit_ledger"] = credit_ledger_valid(tr["cohorts"])
    checks["reward_v3_checkpoint"] = ckpt.get("reward_version") == "completion_credit_v3"
    checks["optimizer_v4_checkpoint"] = ckpt.get("optimizer_version") == "value_unclipped_v4"
    evaluations, seeds, cohorts, diagnostic_rows = [], [], [], []
    for p in args.evaluation:
        m = verify_assets(p)
        r = json.loads((p / "pilot/report.json").read_text())
        seeds.append(r["seed"])
        checks[p.name + "_credit_ledger"] = credit_ledger_valid(r["cohorts"])
        ...
        frozen = json.loads((ROOT / "configs/protocol_v4.json").read_text())
    ...
    result = {"schema": 1, "status": "verified" if all(checks.values()) else "check_failed", "research_result": False,
              ...
              "scope_and_limitations": [
                  "One training seed, 22 changing-policy training cohorts and 84 fixed development requests; not a final-test estimate or a method comparison.",
                  ...
                  "The v3 scripted controls use the same architecture-instantiation RNG path; they are development controls, not an equal-cost learning comparison.",
    ...
    nominal = result["development"]["metrics"]["nominal"]
    all_metrics = result["development"]["metrics"]["all"]
    recovery_bins = [key for key, value in result["development"]["metrics"].items()
                     if key not in {"all", "nominal"} and value["witnessed_recoveries"] > 0]
    result["competence_gate"] = {
        "nominal_at_least_10_of_12": nominal["completions"] >= 10 and nominal["requests"] == 12,
        "fault_at_least_36_of_72": all_metrics["completions"] - nominal["completions"] >= 36 and all_metrics["requests"] - nominal["requests"] == 72,
        "at_least_three_witnessed_recoveries": all_metrics["witnessed_recoveries"] >= 3,
        "at_least_two_fault_bins_with_recovery": len(recovery_bins) >= 2,
        "all_requests_and_artifacts_valid": all(checks.values()) and all_metrics["forbidden_events"] == 0,
    }
    result["competence_gate_passed"] = all(result["competence_gate"].values())
    result["reward_version"] = "completion_credit_v3"
    result["training"]["upstream_mean_return_by_cohort"] = [statistics.mean(c["upstream_discounted_job_returns"]) for c in tr["cohorts"]]
    result["training"]["completion_credit_mean_return_by_cohort"] = [statistics.mean(c["completion_credit_discounted_returns"]) for c in tr["cohorts"]]
    result["training"]["policy_action_std_by_cohort"] = [c["policy_action_std"] for c in tr["cohorts"]]
    result["prior_development_costs"] = json.loads((ROOT / "configs/unclipped_training_registration_v4.json").read_text())["previous_cost_accounting"]
    with args.output.open("x") as f:
        f.write(json.dumps(result, indent=2) + "\n")
    print(json.dumps({"status": result["status"], "competence_gate_passed": result["competence_gate_passed"], "competence_gate": result["competence_gate"], "development": result["development"]["metrics"]}, indent=2))
