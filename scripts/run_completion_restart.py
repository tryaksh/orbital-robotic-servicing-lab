"""Launch the registered substantial uniform run and final-budget development evaluations."""
...
from assembly_recovery.power_guard import system_awake, system_power_status  # noqa: E402
...
def main():
    ...
    parser.add_argument("--cohorts", type=int, default=22)
    ...
    if args.cohorts != 22 or not 1 <= args.max_minutes <= 150:
        parser.error("The registered run requires 22 cohorts with a 1..150 minute wall limit")
    ...
    protocol = ROOT / "configs/protocol_v3_restart_v1.json"
    if not protocol.is_file():
        parser.error("Versioned protocol is missing")
    frozen = json.loads(protocol.read_text())
    if args.mode == "train" and (args.seed != frozen["training"]["seed"] or args.num_envs != frozen["training"]["num_envs"] or args.cohorts != frozen["training"]["cohorts"]):
        parser.error("Training specification differs from the frozen registration")
    ...
    for name, digest in frozen["gate_evidence_sha256"].items():
        if sha256(ROOT / name) != digest:
            parser.error("Frozen gate evidence changed: " + name)
    reward_gate = json.loads((ROOT / "evidence/completion_credit_controls_v3_r01.json").read_text())
    if reward_gate["status"] != "verified" or not all(reward_gate["checks"].values()):
        parser.error("Reward-v3 controls have not passed")
    ...
    if power["platform"] == "windows" and power["ac_line_status"] != 1:
        parser.error("Connect AC power before the registered GPU restart")
    output = ROOT / "artifacts/assembly" / run_id
    output.mkdir(parents=True, exist_ok=False)
    command = [str(args.sim_python), str(ROOT / "scripts/train_completion.py"), "--headless", "--output", str(output / "pilot"),
    ...
    manifest = {"schema": 1, "status": "starting", "run_id": run_id, "research_result": False,
                ...
                "input_checkpoint": checkpoint, "wall_time_limit_s": args.max_minutes * 60, "preflight_power": power,
                "environment_overrides": {"TORCHDYNAMO_DISABLE": "1", "PYTHONUNBUFFERED": "1"},
                "scope": "Fresh registered uniform reward-v3 development run; final-budget evaluation only, no adaptive comparison or final test."}
    ...
    try:
        with system_awake() as power_guard:
            manifest["power_guard"] = power_guard
            write_json(output / "manifest.json", manifest)
