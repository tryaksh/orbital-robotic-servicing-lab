"""Bounded native contact-impact queue with exact prelaunch source archives."""
...
def preflight():
    ...
    frozen = json.loads((ROOT / "configs/learned_physics_validation_v3.json").read_text())
    protocol = json.loads((ROOT / frozen["physical_protocol"]["path"]).read_text())
    if sha256(ROOT / frozen["physical_protocol"]["path"]) != frozen["physical_protocol"]["sha256"]:
    ...
    for mapping in (protocol["behavior_source_sha256"], frozen["source_sha256"],
                    frozen["installed_source_sha256"], frozen["preserved_evidence_sha256"]):
        for name, digest in mapping.items():
            if sha256(ROOT / name) != digest:
                raise ValueError("Protected source/evidence changed: " + name)
    checkpoint = ROOT / frozen["checkpoint_path"]
    if sha256(checkpoint) != frozen["checkpoint_sha256"]:
        raise ValueError("Complete ordinary model changed")
    return frozen
...
def main():
    ...
    parser.add_argument("--refinements", type=int, nargs="+", default=[1, 2, 4], choices=(1, 2, 4, 8))
    parser.add_argument("--extension-evidence", type=Path)
    ...
    if 8 in args.refinements and (
        not args.extension_evidence or not json.loads(args.extension_evidence.read_text())["decision"]["requires_960"]
    ):
        raise ValueError("960 Hz requires the registered finer-pair disagreement evidence")
    if args.plan:
    ...
    for refinement in args.refinements:
        ...
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/probe_contact_impact_v1.py"),
        ...
        try:
            ...
            if manifest["status"] == "completed":
                from scripts.review_contact_impact_v1 import verify_single
