"""Versioned provenance correction: tests verified from the clean prelaunch commit."""
...
def verify_single_v1(path, registration):
    ...
    checks = {
        ...
        "source_and_registration": manifest["registration_sha256"] == sha256(REGISTRATION)
            and all(manifest["source_hashes"][p] == h for p, h in registration["source_sha256"].items() if not p.startswith("tests/")),
        "test_source_from_clean_prelaunch_commit": not manifest["source_dirty_at_start"] and all(
            hashlib.sha256(subprocess.check_output(["git", "-C", str(ROOT), "show", manifest["source_commit_at_start"] + ":" + p])).hexdigest() == h
            for p, h in registration["source_sha256"].items() if p.startswith("tests/")),
...
def run_path_v2(root, label):
    queue = json.loads((root / "queue.json").read_text())
    if label == "120_prefix_learned" and queue.get("reused_first_run"):
        return ROOT / queue["reused_first_run"]["path"]
    return root / label
...
def summarize_pairs_v1(root, registration):
    ...
    for row in registration["run_order"]:
        label = row["label"]
        result = verify_single_v1(run_path_v2(root, label), registration)
        results[label] = result
        _, reports[label], traces[label] = load_run(run_path_v2(root, label))
        checks[label] = result["status"] == "verified"
    queue = json.loads((root / "queue.json").read_text())
    continuation_path = ROOT / "configs/learned_physics_continuation_v2.json"
    checks["continuation_queue_complete"] = queue["status"] == "completed" and len(queue["runs"]) == 6
    checks["continuation_registration"] = queue["execution_registration_sha256"] == sha256(continuation_path)
