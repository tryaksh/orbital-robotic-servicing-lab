"""Versioned frozen learned/retry physics validation; no optimization or final tests."""
...
def main():
    ...
    parser.add_argument("--mode", choices=("prefix_learned", "retry", "direct_learned"), required=True)
    ...
    args.controller = "retry" if args.mode == "retry" else "learned"
    args.output.mkdir(parents=True, exist_ok=False)
    report = {"status": "starting", "controller": args.controller, "seed": args.seed,
               "research_result": False, "reward_version": "completion_credit_v3", "diagnostic_protocol": "learned_physics_validation_v3", "mode": args.mode, "refinement": args.refinement, "training_run_status": "completed", "checkpoint_selection": "Final registered 22-cohort budget checkpoint; no selection by score"}
    app = env = None
    try:
        ...
        from assembly_recovery.physics_validation_v1 import whole_job_summary_v1
        ...
        env = PegLearnedRefinementEnvV2(cfg, criteria=criteria, cases=cases, nominal=study["fault_support"]["nominal"], audit=True, refinement=args.refinement)
        ...
        report.update(controller_position_bounds=list(cfg.ctrl.pos_action_bounds),
                      ...
                      controller_schedule="Direct: frozen mean from control zero. Prefix: unchanged script through control 59 then frozen mean. Retry: unchanged throughout, reused for both analyses without duplicate cost or independent requests.",
        ...
        for step in range(450):
            with torch.no_grad():
                ...
                if args.mode == "retry" or (args.mode == "prefix_learned" and step < 60):
        ...
        artifact.update(initial=initial, sensor_initial=sensor_initial, physical_initial=physical_initial,
                        ...
                        physics=env.audit_buffer.cpu(), sensor_draws=torch.cat(draws).cpu(),
        ...
        endpoints = [post_stall_completion(job, w, criteria, handoff_step=env.timing.handoff_step) for job, w in zip(jobs, witness, strict=True)]
        report.update(post_stall_endpoint=endpoints, post_stall_summary=summarize_endpoints(endpoints, jobs),
                      whole_job=whole_job_summary_v1(jobs),
                      servo_updates=env.servo_updates, sensor_updates=env.sensor_updates,
                      native_physics_steps=env.executed_job_steps,
                      expected_total_simulated_seconds=30.,
                      scope_and_limitations=[
                          "One complete frozen model, one development seed, all 28 fixed cases. No training, tuning or final-test access.",
                          "Both resolutions independently initialize at the confirmed 120 Hz before a single between-job configuration of the explicit native integration argument. No USD timestep mutation, tensor-view reconstruction, pose restoration, filter refresh, reseed, or attachment change. This tests job-time physics sensitivity conditional on coarse initialization, not fully refined initialization or convergence.",
                          "15 Hz policy, 120 Hz external servo/sensor and unchanged implicit native gripper solver; native raw 20 N abort, retained grasp, 30-second deadline and 0.5-second dwell.",
                          "Direct and fixed-prefix jobs remain separate assays. The unchanged retry trajectory is reused analytically, never counted as independent duplicate requests or simulator work.",
                          "The supplemental post-stall endpoint is interpreted only for the prefix assay. Direct outcomes primarily measure prevention; direct post-stall records are diagnostic only.",
                          "Original withdrawal metric and failed competence gate remain frozen. Two timesteps show sensitivity, not convergence or hardware safety."])
