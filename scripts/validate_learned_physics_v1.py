...
def main():
    ...
    report = {"status": "starting", "controller": args.controller, "seed": args.seed,
               "research_result": False, "reward_version": "completion_credit_v3", "diagnostic_protocol": "learned_physics_validation_v1", "mode": args.mode, "refinement": args.refinement, "training_run_status": "completed", "checkpoint_selection": "Final registered 22-cohort budget checkpoint; no selection by score"}
    app = env = None
    try:
        ...
        env = PegLearnedRefinementEnvV1(cfg, criteria=criteria, cases=cases, nominal=study["fault_support"]["nominal"], audit=True, refinement=args.refinement)
        ...
        report.update(criteria=asdict(criteria), geometry=env.geometry.report(),
                      ...
                      actual_solver_dt=env.sim.get_physics_dt(),
        ...
        report.update(post_stall_endpoint=endpoints, post_stall_summary=summarize_endpoints(endpoints, jobs),
                      ...
                      scope_and_limitations=[
                          ...
                          "Both resolutions independently initialize at the confirmed 120 Hz before a single between-job timestep change. No pose snapshot restoration, filter refresh, reseed, or attachment change. This tests job-time physics sensitivity conditional on coarse initialization, not fully refined initialization or convergence.",
