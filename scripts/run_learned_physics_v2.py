"""Continue five frozen runs after the preserved v1 CPU provenance-check failure."""
...
def main():
    ...
    registration_path = ROOT / "configs/learned_physics_validation_v1.json"
    registration = json.loads(registration_path.read_text())
    continuation_path = ROOT / "configs/learned_physics_continuation_v2.json"
    continuation = json.loads(continuation_path.read_text())
    if continuation["physics_registration_sha256"] != sha256(registration_path):
        raise ValueError("Original physics registration changed")
    for p, h in continuation["source_sha256"].items():
        if sha256(ROOT / p) != h:
            raise ValueError("Continuation source changed: " + p)
    prior = continuation["reused_first_run"]
    if sha256(ROOT / prior["path"] / "manifest.json") != prior["manifest_sha256"]:
        raise ValueError("Preserved first simulator run changed")
    verification = ROOT / prior["path"] / "verification_v2.json"
    if sha256(verification) != prior["verification_sha256"] or json.loads(verification.read_text())["status"] != "verified":
        raise ValueError("First 120 Hz run must pass corrected CPU and compatibility checks")
    if args.plan:
    ...
    queue = {"schema": 1, "status": "running", "runs": [], "registration_sha256": sha256(registration_path),
             "execution_registration_sha256": sha256(continuation_path), "reused_first_run": prior, "launcher_pid": os.getpid(), "started_at_utc": datetime.now(UTC).isoformat(),
             "wall_limit_s": continuation["wall_limit_s"], "run_order": registration["run_order"]}
    queue["runs"].append({"label": "120_prefix_learned", "status": "verified", "reused": True,
                          "manifest_sha256": prior["manifest_sha256"], "verification_sha256": prior["verification_sha256"]})
    write_json(output / "queue.json", queue)
    deadline = time.monotonic() + continuation["wall_limit_s"]
    for row in registration["run_order"][1:]:
        preflight_v1(registration)
        ...
        checkpoint = ROOT / registration["checkpoint_path"]
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/validate_learned_physics_v1.py"), "--headless",
        ...
        manifest = {"schema": 1, "status": "starting", "run_id": args.run_id + "/" + label,
            ...
            "registration_sha256": sha256(registration_path), "execution_registration_sha256": sha256(continuation_path), "environment_lock": json.loads(lock.read_text()),
        ...
        if manifest["status"] == "completed":
            verification_command = [sys.executable, str(ROOT / "scripts/verify_learned_physics_v2.py"),
