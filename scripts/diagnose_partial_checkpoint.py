"""Frozen-policy development diagnosis; no optimization or task changes."""
...
def main():
    ...
    parser.add_argument("--controller", choices=("deterministic", "stochastic", "clipped_mean", "retry"), required=True)
    parser.add_argument("--checkpoint", type=Path, required=True)
    parser.add_argument("--checkpoint-sha256", required=True)
    AppLauncher.add_app_launcher_args(parser)
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=False)
    report = {"status": "starting", "controller": args.controller, "seed": 10070,
              "policy_rng_seed": 310070, "research_result": False, "reward_version": "completion_credit_v3", "diagnostic_protocol": "partial_checkpoint_diagnosis_v1", "training_run_status": "failed_incomplete", "checkpoint_selection": "Last durable checkpoint before external interruption, cohort 13; not a completed-budget policy"}
    app = env = None
    try:
        ...
        if loaded["status"] != "partial" or loaded["completed_cohorts"] != 13 or loaded["spec"] != model.spec:
            raise ValueError("Expected frozen partial cohort-13 checkpoint from the interrupted run")
        model.load_state_dict(loaded["model"])
        model.eval()
        generator = torch.Generator(device=env.device).manual_seed(report["policy_rng_seed"])
        ...
        controllers = [ActorRetryController(row, step_dt=env.step_dt, position_bounds=cfg.ctrl.pos_action_bounds,
                       seated_height_m=env.geometry.seated_fingertip_above_hole_top_m, retry=True, settings=settings)
        ...
        for step in range(450):
            with torch.no_grad():
                ...
                if args.controller == "clipped_mean":
                    action = expected_clipped_action(mean, model.log_std.clamp(-5, 2).exp())
                elif args.controller == "stochastic":
                    action, mean = independent_action(model, observation, generator)
                elif args.controller == "retry":
        ...
        report.update(jobs=jobs, recovery_witness=witness, discounted_job_returns=returns.cpu().tolist(),
                      ...
                      scope_and_limitations=["All 28 registered development cases; no optimization or final-test use.",
                          "Separate stochastic-policy RNG; exact raw sensor draws saved for paired verification.",
                          "Unchanged scripted action algorithm; generic target-based contact-witness intent shared across controllers.",
