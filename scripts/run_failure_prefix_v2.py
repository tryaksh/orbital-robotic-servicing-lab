"""Versioned six-run confirmation using the existing bounded serial prefix launcher."""
...
def main():
    ...
    protocol = ROOT / "configs/protocol_v4.json"
    ...
    checkpoint = ROOT / "artifacts/assembly/uniform_unclipped_170_v4_r02/pilot/budget_checkpoint.pt"
    registration = ROOT / "configs/failure_prefix_confirmation_v2.json"
    ...
    if training["status"] != "completed" or sha256(checkpoint) != registered["checkpoint_sha256"]:
        parser.error("Expected the registered complete training checkpoint")
    if registered["seeds"] != [10071, 10072] or registered["controllers"] != ["learned", "retry", "continue"]:
        parser.error("Only the registered two development seeds and three frozen controllers are permitted")
    if sha256(protocol) != registered["physical_protocol"]["sha256"]:
        parser.error("Frozen physical protocol changed")
    for name, digest in registered["prior_evidence_sha256"].items():
        if sha256(ROOT / name) != digest:
            parser.error("Preserved prior evidence changed: " + name)
    output.mkdir(parents=True, exist_ok=False)
    queue = {"status": "running", "runs": [], "registration_sha256": sha256(registration),
             "expected_charged_transitions": 77007., "wall_limit_s": 2520}
    write_json(output / "queue.json", queue)
    deadline = time.monotonic() + queue["wall_limit_s"]
    for seed, controller in ((seed, controller) for seed in registered["seeds"]
                             for controller in registered["controllers"]):
        run = output / f"{seed}_{controller}"
        run.mkdir(exist_ok=False)
        command = ["C:/isaac-sim/python.bat", str(ROOT / "scripts/diagnose_failure_prefix_v2.py"), "--headless",
                   "--output", str(run / "diagnosis"), "--controller", controller,
                   "--seed", str(seed), "--checkpoint", str(checkpoint), "--checkpoint-sha256", sha256(checkpoint)]
        lock = ROOT / "environment-lock.local.json"
        manifest = {"schema": 1, "status": "starting", "run_id": args.run_id + f"/{seed}_{controller}",
                    "research_result": False, "seed": seed, "controller": controller, "started_at_utc": datetime.now(UTC).isoformat(),
                    ...
                    "expected_charged_transitions": 12834.5, "wall_limit_s": 420,
                    "scope": "Separate action-only failure-prefix recovery assay. Three controllers, same physical task and reward-v3; does not replace the original competence gate."}
        ...
        try:
            code, timeout, resource_stop = run_resource_bounded(command, run / "process.log", min(deadline, start + 420),
            ...
            checks = {"completed_report": report.get("status") == "completed",
                      "registered_seed": report.get("seed") == seed,
                      "endpoints_saved": len(report.get("post_stall_endpoint", [])) == 28, "all_requests": len(report.get("jobs", [])) == 28,
            ...
            write_json(run / "manifest.json", manifest)
        queue["runs"].append({"seed": seed, "controller": controller, "status": manifest["status"], "manifest_sha256": sha256(run / "manifest.json")})
        write_json(output / "queue.json", queue)
        print(json.dumps(queue["runs"][-1]), flush=True)
        if manifest["status"] != "completed":
            break
    queue["status"] = "completed" if len(queue["runs"]) == 6 and all(r["status"] == "completed" for r in queue["runs"]) else "failed_or_incomplete"
    write_json(output / "queue.json", queue)
