...
def main():
    ...
    original = torch.load(ROOT / "artifacts/assembly/uniform_dev_10070_v3_r02/pilot/diagnostics_0.pt", weights_only=True, map_location="cpu")
    reference = traces["deterministic"]
    checks["original_budget_evaluation_physics_reproduced"] = torch.equal(reference["physics"], original["physics"])
    checks["original_budget_evaluation_actions_reproduced"] = torch.equal(reference["applied_action"], original["controls"][:, :, 10:17])
    ...
    result = {"schema": 1, "status": "verified" if all(checks.values()) else "check_failed", "research_result": False,
              "registration_sha256": sha256(ROOT / "configs/extraction_diagnosis_v2.json"), "checks": checks, "arms": arms, "stochastic_critic_calibration": calibration,
              ...
              "scope_and_limitations": ["Four distinct controllers on every registered seed-10070 development case, with exact initial state and random draws.",
                  "Original v3 deterministic evaluation remains the registered primary development result; no retrospective substitution.",
