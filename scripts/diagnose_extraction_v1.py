...
def main():
    ...
    report = {"status": "starting", "controller": args.controller, "seed": 10070,
              "policy_rng_seed": 310070, "research_result": False, "reward_version": "completion_credit_v3", "diagnostic_protocol": "extraction_diagnosis_v1"}
    app = env = None
    try:
        ...
        if loaded["status"] != "budget_complete" or loaded["spec"] != model.spec:
            raise ValueError("Expected frozen complete pilot checkpoint")
        ...
        names = ("actor_before", "mean_action", "requested_action", "applied_action", "target_xyz", "tool_xyz",
                 "part_relative_xyz", "part_quat", "raw_reward", "job_reward", "active", "dead_zone", "completion_credit", "upstream_job_reward")
        ...
        for step in range(450):
            with torch.no_grad():
                mean = model.act(observation, deterministic=True)[0]
