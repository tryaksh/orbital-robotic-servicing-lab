...
The first cable cycle is closed by `evidence/cable_cycle_decision_v1.json`: native force-guided seating is competent on its six development cases, but the rigid SC model fails the matched axial-retention gate. Read the cable section of ROADMAP first. The next action is a physically justified retention/load model with positive, negative and release controls, followed by feasible prior-clip/snags; do not start candidate training or call held seating a completed retained connection. Existing peg physics/prefix failures remain closed separately. Native setup uses `scripts/setup_cable.py`; bounded workers use `scripts/run_cable.py`. The recorded cable runtime is CPU MuJoCo, not the old 2,048-environment peg capacity result.
