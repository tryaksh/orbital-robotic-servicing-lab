...
| Training | V2 pilot complete at 2,346,880 charged transitions; v3 attempt incomplete after 13/22 cohorts. The corrected v4 attempt also ended early after seven cohorts; all seven partial model/optimizer checkpoints are finite. At least 3,556,352 charged v4 transitions are preserved as failed-run cost. No full-budget v3/v4 checkpoint exists yet. |
...
| Single next action | Repeat the same registered fresh v4 run as uniform_unclipped_170_v4_r02 through the existing bounded launcher (1024 environments, 22 cohorts, 150-minute cap), then evaluate only its final budget checkpoint on all three development seeds. The previous process is confirmed stopped; preserve all failed-run costs. No adaptive campaign before competence. |
