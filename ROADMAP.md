...
| Remaining validation | Peg pilot protocol v2 remains frozen; sampled support is validated without continuous-range guarantees. New post-stall confirmation supports bounded learned-policy physics refinement. Prior scripted 120/240 Hz pairs remain sensitive (2/4 versus 1/4), not converged. Learned physics robustness, shared uniform/adaptive/fixed-curriculum registration, multiple training seeds, gear and final tests remain open. |
...
| Resource allocation | This block completed exactly six serial simulator runs and 77,007 charged transitions, with no training or extra GPU probes. Summed worker time including initialization/export is 992.92 s; peak GPU 4,377 MiB and minimum free RAM 14,338 MiB. Cumulative tracked ledger is at least 24,841,670.25 transitions. CPU replays/export corrections add zero simulator cost. All owned simulator jobs have stopped. |
...
| Single next action | Preregister a bounded learned-policy physics-refinement comparison at 120/240 Hz with the frozen v4 mean and unchanged retry, preserving 15 Hz policy, 120 Hz servo/sensor timing, native 20 N abort, 30-second jobs and 0.5-second dwell. First verify the 120 Hz evaluation adapter against the frozen behavior and explicitly pair noise, nuisance parameters and initializations; report initialization differences and contact sensitivity rather than claiming convergence. Retain both recovery metrics and keep final tests closed. No adaptive campaign before learned-physics validation and a newly registered shared uniform/adaptive/fixed-curriculum configuration. |
