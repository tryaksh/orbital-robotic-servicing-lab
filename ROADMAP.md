...
| Verification | Ruff and 173 CPU tests pass. Four-arm partial diagnosis and controlled CPU critic-fit artifacts verified, including exact source snapshots and unchanged actor/normalizers. Frozen v1/v2/v3 hashes retained; v4 changes only value-loss clipping. |
| Study implementation | Reward-v3 finite-job PPO retained. Controlled equal-update critic regression reduced RMSE from 222.58 with value clipping to 10.43 without clipping. Fresh uniform v4 is registered at the same 22-cohort budget; actor clipping, physical task, reward, architecture and competence gate are unchanged. No sleep guard is used. |
...
| Scientific evidence | Original v2 pilot remains 0/84. The v3 partial cohort-13 policy completes 12/28 deterministic, 11/28 stochastic and 11/28 expected-clipped-action requests; every learned arm fails all four nominal cases and has zero witnessed recoveries. Matched retry completes 19/28 with 15 witnessed recoveries. Stochastic critic prediction mean 10.55 versus realized return-to-go 288.58 motivates the verified optimizer correction. Partial diagnostics are not primary competence evidence. |
| Resource allocation | 40 serial GPU launches: 36 complete, 4 failed/incomplete; cumulative charged transitions at least 10,805,032.25. Renewed block completed four matched 28-case diagnosis arms (51,338 charged transitions) and a zero-simulator-cost CPU critic-fit experiment. V4 fresh training will charge 10,326,272 plus 38,503.5 for the fixed development evaluation. |
...
| Single next action | Launch registered fresh uniform value-unclipped v4 (seed 170, 1024 environments, 22 cohorts, 10,326,272 charged transitions, 150-minute cap); inspect durable checkpoints without selecting by score. Evaluate only the final budget checkpoint on all three declared development seeds, verify artifacts and the unchanged competence gate. No adaptive campaign before competence. |
