...
| Verification | Ruff, 144 CPU tests and both launch plans pass without Isaac Sim or ignored local artifacts; an export of the staged Git tree also passes, including byte-exact frozen protocol hashes and import isolation. All 28 GPU-run artifact/source snapshots rehashed. See evidence/tensorized_training_block_v2.json and evidence/git_checkout_verification_v2.json. |
| Study implementation | GPU finite-job evaluator, separate feedforward actor/critic PPO, terminal observation/reward/GAE masks, uniform sampler, generic contact-recovery witness and runtime mutation guards implemented. Exact CPU/tensor scripted trajectories, outcomes and rewards verified before and after the initial-action correction. Upstream source unchanged. |
...
| Scientific evidence | The budget-selected deterministic learned policy completes 0/84 development requests: 80 deadlines, 4 force aborts, 0/12 nominal and zero witnessed recoveries; all denominators retained. Scripted support remains 73/96 versus 18/96 on the boundary/interior grid. These are separate controller results, not a paired learning comparison. See evidence/uniform_pilot_v2.json. |
...
| Execution authorization | Owner authorizes autonomous bounded implementation and GPU work with minimal progress messages. This block is finished; no GPU job is left queued. Verified implementation and evidence committed on local branch research/assembly-recovery-training; raw weights/traces remain ignored local artifacts. GitHub has not been changed or pushed; the existing origin still names the retired project. |
| Single next action | Implement and run the registered 38,503.5-transition development diagnosis in evidence/pilot_failure_diagnosis_v2.json: frozen deterministic policy, independently sampled stochastic policy and unchanged retry on the same 28 cases with matched initialization/sensor RNG. Compare complete jobs, action targets and full returns to distinguish policy extraction/undertraining from reward incentives before more training or adaptive curricula. No script tuning, geometry/force relaxation or final-test opening. |
