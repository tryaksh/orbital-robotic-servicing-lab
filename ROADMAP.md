...
| Verification | Lint and 68 CPU tests pass. All three new runs have verified prelaunch source snapshots and artifact hashes; saved physics and actor-only controller decisions replay exactly. The paired initial states, resolved environment YAML and 60 approach control steps are identical. The 450-frame comparison video decodes completely; lint, bare pytest and planning also pass from a tracked-source export without ignored local artifacts |
| Study implementation | Peg evaluator, actor-only scripted withdrawal/search/reinsertion controller, per-term upstream reward recorder and offline pair verifier. Eleven current validation runs / 44 scripted jobs; no learned recovery policy |
| Physics and job accounting | Physical held-part gravity selected; all 12 current gravity-enabled holds retained the grasp. Finger-contact scoring catches all 4 deliberate releases, including the near-hand release missed by the old distance-only check. Zero automatic resets; 12-second reset-horizon probe passed |
| Remaining validation | One complete recovery/reward pair passes the narrow development gate. Numeric fault support, training-wrapper information/terminal handling and protocol freeze remain open. No long training yet |
| Scientific evidence | Seed 10070 case 2: contact stall at 2.933 s, actor retry at 4 s, 12.368 mm withdrawal, successful 0.5 s seating dwell at 13.75 s. Matched continued insertion times out at 30 s. Discounted returns: 75.797 vs 37.543, gamma 0.995. All four jobs per arm retained: retry 2 success / 1 deadline / 1 force abort; continued 1 / 2 / 1. One unique recovered case, repeated once; no learning effect or held-out result. See evidence/recovery_pair_10070_r02.json |
| Resource allocation | Three serial GPU probes in this block, 12 jobs / 5,400 control transitions including probe and post-terminal holds. Recorded rollouts measured 12.94 and 13.03 transitions/s with CPU traces/video; observed GPU memory about 6.8 GB. These are validation costs, not training capacity |
...
| Execution authorization | Owner explicitly authorizes Assembly Recovery Lab, overriding obsolete camera-reliability/no-recovery instructions. No owner action or manual intervention is currently required |
| Single next action | Define and validate numeric recoverable fault bins on development seeds. Start with an actor-only unload-and-realign probe of case 1's unresolved >3.25 mm lateral miss; rerun the paired continued-insertion comparator under the same geometry, observations, 30 s deadline and 20 N wrist budget. Keep test seeds closed and finish training-wrapper audits before protocol freeze or long training |
...
**Next block acceptance:** declare numeric development fault bins derived from unchanged geometry and action limits, then verify initialization, contact failure and same-job recovery support with the actor-only controller. Start with the unresolved lateral miss and distinguish insufficient lateral movement under load from inadequate search coverage. Preserve every failed attempt and pair any changed controller with continued insertion; do not inspect final-test seeds. The recorded recovery in evidence/recovery_pair_10070_r02.json establishes one feasible case, not the full support. Keep the same research question and robot.
