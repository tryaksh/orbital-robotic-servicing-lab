...
2. Resolve held-part gravity: run matched short trials with upstream gravity disabled and with it enabled; examine grasp retention and contact forces. Use physical held-part gravity for the primary industrial study if the task passes validity checks. If that is blocked, time-box the repair to two work blocks and explicitly constrain the paper to the upstream benchmark rather than pretending physical fidelity. Preserve both results. This decision precedes all primary training.
