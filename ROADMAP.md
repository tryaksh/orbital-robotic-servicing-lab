...
| Verification | Lint and 58 CPU tests pass; final saved physics traces reloaded and replayed exactly; artifact hashes verified. Clean tracked-file export checked before handover |
| Study implementation | Peg evaluator runs in Isaac Sim with physics-step scoring, direct fixture/finger contact, reset/write guards and corrected velocity observations. Eight current validation runs / 32 scripted jobs; no learned recovery policy |
...
| Remaining validation | Full recovery/reward comparison, numeric fault support, retry/training information audit and protocol freeze remain open. No long training yet |
| Scientific evidence | Seven sustained contact stalls were withdrawn with retained grasp and ended clear of the fixture. One first-attempt scripted seating. Zero completed recoveries; no method comparison or held-out results. See evidence/peg_validation.json |
...
| Single next action | Produce one complete, recorded stall -> withdraw -> reinsert job using allowed observations, paired against continued insertion from the same initial case. Compare full discounted reward, then define recoverable fault bins. Do this before training |
...
**Next block acceptance:** save a complete recovery and a matched continued-attempt trajectory, including unsuccessful jobs, source/config hashes, per-term reward, and a short recorded comparison. Use the current development cases; do not inspect final-test seeds. The retry trigger must use noisy actor-available force and pose, not the evaluator-only fixture sensor or filtered-load stall flag. If the bounded controller cannot finish, report the measured cause and the next targeted change. No robot or research pivot.
