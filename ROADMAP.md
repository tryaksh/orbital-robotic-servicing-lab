...
| Single next action | Run the registered failure_prefix_recovery_v1 assay: the same four-second scripted first attempt, then frozen learned mean, unchanged retry, or continued insertion on 28 development cases per arm. Verify identical physical prefixes and noise, all-request accounting, and recovery only after witnessed prefix failure. Primary 81/84 evaluation contained zero witnessed failures, so its original recovery gate remains failed. No adaptive campaign from this supplemental one-seed assay alone. |
