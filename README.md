...
These are infrastructure checks. The peg adapter now runs with direct part/fixture and finger-contact sensing, complete-job scoring and blocked resets/pose writes. A scripted controller using only noisy actor measurements completes one recorded contact stall -> withdrawal -> reinsertion at 13.75 seconds. Its paired continued-insertion controller starts from an identical condition and reaches the 30-second deadline. The full four-case cohort retains a shared force abort and an unrecovered deadline case. The release check catches all four deliberately released parts. **The learned method, numeric fault support, trained baseline and website remain unfinished.** This is one development recovery case, not evidence of a training advantage. See [evidence/recovery_pair_10070_r02.json](evidence/recovery_pair_10070_r02.json), [evidence/peg_validation.json](evidence/peg_validation.json), and ROADMAP.md.
