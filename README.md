...
| Perception on a two-bay rack | Occupancy solved; **the camera arm passes its gate**, after the 2026-08-17 result was retracted | The pose head gains a bay-occupancy output and reads *which* bay holds the module at **100% exact-match** on 12,000 held-out frames against a 66.6% majority-class baseline. Re-certified 2026-08-18 over 576 workflows per arm on three held-out seeds: oracle **88.72%**, camera **84.90%**, blind **34.03%**. Camera is **3.82 points** behind oracle against a 10-point allowance and **50.9 points** above blind, so both halves of the gate pass. The superseded run reported 65.10% and a 23.6-point gap, and all of that difference was one seed that scored 25.00% and re-runs at 84.90%. **The caveat that replaces it:** the camera arm is not deterministic — six runs span 80.73–86.46% with a 2.13-point standard deviation, while the oracle arm is bit-identical run to run (`evidence/vision_camera_run_variance.json`) — so a single camera run is one draw, and the retracted outlier sits 27.6 standard deviations outside even that spread, which is a second mechanism rather than a bad draw of the first, and it is unidentified. |
