...
The project now has a verified tensorized finite-job training path around pinned FORGE: complete-job scoring, actor/critic separation, terminal rewards and returns, contact sensing, mutation guards and transition accounting. Lint and 173 CPU tests pass in source-only and staged-Git exports. Protocol v2 is frozen in `configs/protocol_v2.json`; upstream source remains unchanged.
...
The study retains physical held-part gravity, the 30-second deadline, raw 20 N wrist-load abort and 0.5-second seating dwell. Robot gravity compensation is idealized; observations are simulator poses with synthetic noise. Four scripted development pairs complete 2/4 at 120 Hz physics and 1/4 at 240 Hz despite matched external timing/noise, demonstrating sensitivity rather than convergence. No pickup, dropped-part recovery, camera perception, hardware transfer or force-certified safety is established. Final tests and gear learning remain unopened. A matched 28-case diagnosis subsequently found 0 deterministic, 5 stochastic and 19 scripted completions. Failed deterministic jobs earned more full return than 10 of the 19 matched successful scripts. A reward-only completion credit removes those observed incentive counterexamples; reruns preserve all physical trajectories bit for bit. The substantial v3 run was interrupted after 13 of 22 cohorts: Windows slept following a lid/button event, and the RAM guard stopped training after wake. All 13 partial checkpoints are preserved and finite; no full-budget policy or new primary evaluation exists. A matched partial-checkpoint diagnosis found 12/28 deterministic completions, no nominal completions and no witnessed learned recoveries. A controlled CPU test isolated slow critic fitting under absolute value clipping; the same update budget gives RMSE 222.58 with clipping versus 10.43 without. A fresh run changing only value clipping is registered, with the original competence gate; learned competence remains unproven. See [evidence/critic_fit_v1.json](evidence/critic_fit_v1.json). See [evidence/uniform_completion_v3_r01_failure.json](evidence/uniform_completion_v3_r01_failure.json). See [evidence/diagnosis_decision_v3.json](evidence/diagnosis_decision_v3.json). ROADMAP.md contains the single next action.
