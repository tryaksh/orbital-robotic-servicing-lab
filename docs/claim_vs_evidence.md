...
| Extraction works one time in ten, and did not work at all before | Extract v6: 908 / 9,001 held-out episodes, 10.09%, Wilson 95% [9.48, 10.73], consistent across three seeds. Chained removal: 81 / 576, 14.06% | `evidence/grapple_extract_v6_certification.json`, `evidence/workflow_remove_clock_certification.json`. Far below the 95% gate. The change was the episode going 15 s → 25 s with a fine-tune: successful extractions take 18.23 s, so the old episode made them impossible by construction. Lengthening the clock *without* retraining was measured first and does not work |
...
| One servicing skill passes its promotion gate, and the chain it sits in reaches 86% | Insert v6: 2,867 / 3,000 held-out episodes, 95.57%, gate passed. Chained installation: 497 / 576, 86.28%, Wilson 95% [83.23, 88.85] | `evidence/grapple_insert_v6_certification.json`, `evidence/workflow_install_v6insert_certification.json`. The whole improvement over v5's 6.96% came from one change: the episode went from 12 s to 20 s, because successful insertions take a median of 13.43 s and the old episode ended before the median success happened |
