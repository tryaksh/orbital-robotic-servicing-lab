...
| ~~The camera arm holds up on a two-bay rack~~ **NOT ESTABLISHED. The gate fails, on one seed of three** | Oracle **88.72%**, camera **65.10%**, blind **34.03%**, 576 workflows each on three held-out seeds. Camera is 23.6 points behind oracle against a 10-point allowance. Per seed it is **86.46% / 25.00% / 83.85%** while oracle is flat at 90.62 / 89.58 / 85.94 | `evidence/vision_workflow_*_twoslot_certification.json`. Two seeds are inside the gate with room and one collapses, so this is an estimator that fails on a randomization draw rather than one that costs 23 points. Not the occupancy readout — that is 100% exact-match and the failures are insertion timeouts, not wrong-bay attempts — and not the manipulation, since oracle is stable across the same seeds. The head's p95 is 6.47 mm against a 4 mm insertion lateral tolerance while its mean is 2.81 mm: an adequate typical accuracy with an inadequate tail. **The same sweep at one seed reported camera 84.90% against oracle 90.63% and would have been published as a pass** |
| The blind control still separates, so the image carries real information | Blind 34.03% against camera 65.10% on the two-bay rack, a 31-point margin, consistent across all three seeds at 31.77 / 37.50 / 32.81 | The blind arm is told the module is where the rack nominally presents it. That half of item 6's gate passes; only the camera-versus-oracle half fails |
