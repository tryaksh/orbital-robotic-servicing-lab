...
| **The camera arm holds up on a two-bay rack, and the cost of perception there is about 4 points** | Re-certified 2026-08-18, 576 workflows per arm on three held-out seeds: oracle **88.72%**, camera **84.90%**, blind **34.03%**. Camera per seed 86.46 / 84.90 / 83.33; oracle 90.62 / 89.58 / 85.94 | `evidence/vision_workflow_*_twoslot_certification.json`. Camera is 3.82 points behind oracle against a 10-point allowance and 50.9 above blind, so both halves pass. **This replaces a retracted result**: the 2026-08-17 run reported 65.10% and a failed gate, and all of the difference was one seed at 25.00% that re-runs at 84.90%. The caveat that replaces it is real — the camera arm is not deterministic. Six runs span 80.73–86.46%, sd 2.13 points, while the oracle arm is bit-identical run to run (`evidence/vision_camera_run_variance.json`), so the pooled Wilson interval understates the uncertainty on any single camera certification |
