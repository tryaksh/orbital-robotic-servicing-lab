# Status, as of 2026-08-24 — SUPERSEDED

> **This is an archived state file. Do not read it for current state and do not
> quote a number from it as current.** It was the canonical status until
> 2026-08-25, when [`../NOW.md`](../NOW.md) replaced it and
> [`../NEXT_WORK.md`](../NEXT_WORK.md) took over its "Open" section.
>
> It is kept because its long-form derivations are not repeated in `NOW.md`: the
> extract attribution ladder step by step, the 12.0 mm pin-feed measurement over
> 433 successful extractions, the audit that found the insert skill and the chain
> describing different problems, and the reasoning behind each. Read it for *why*,
> never for *what is true now*.

The verified state of this repository as it stood on 2026-08-24, branch
...
`docs/handover/` holds the session handoffs in the order they were written. They
are kept for the reasoning and the negative results in them, not for their status
claims — every one has been superseded by this file. `docs/handover/README.md`
