...
| Gripper-to-tool-frame calibration | A 165.6 mm offset between the commanded tool frame and the real finger pads made physical grasping impossible while every pose metric still read near zero | Derive the tool offset from the pad geometry, verify with `scripts/grasp_diagnostics.py`, and keep the check in the grasp gate |
