...
| Contact force sensing | Net PhysX normal force on the blade, in tool axes, raw plus a 100 ms first-order filter, scaled by 20 N | Every control step, force-feedback task only | Wrist force/torque sensor on a real servicing arm | Actor, on the force-feedback task only | Cut contact impulse 59% mean against a matched no-feedback control; no bias, drift, quantization, or mounting compliance is modelled |
...
| A force/torque sensor model rather than ground-truth contact | The policy currently reads exact PhysX contact force, which no real load cell provides | Add bias, drift, quantization, bandwidth, and mounting compliance, then re-certify the force-feedback policy under them |
| Grasp-pose reach calibration | The handle was configured 0.179 m from the flange while the fingers reach only to about 0.15 m, making physical grasping impossible while every pose metric still read near zero | Derive the tool offset from the pad geometry, verify with `scripts/grasp_diagnostics.py`, and keep the check in the grasp gate |
| Workcell placement tolerance | The base position is treated here as an exact number, and it is not a free one: the arm holds the head-on approach attitude only outside a boundary that sits a fixed 0.4242 m in front of the base and moves with it one for one. A cell built 50 mm nearer than specified loses 50 mm of the depth every extraction and transit needs, and the failure is silent because the arm still *reaches* those depths -- it reaches them pointing the wrong way, and pays for it later as grip attitude | Sweep the base as a randomized parameter rather than a constant, the way mass and friction already are, and certify across the tolerance band a real mount would hold. `scripts/solve_workcell.py` measures the band; nothing yet trains across it |
