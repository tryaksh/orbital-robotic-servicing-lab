...
## Paying extraction to arrive settled works, and the module rotates instead

The experiment the record has been pointing at since the velocity limits were
derived. Nothing in the extract objective paid the policy to arrive settled:
`extraction_success_mask` asks for a module that is clear **and** under the
derived limits, every dense term was about travel, and velocity entered only
through a sparse terminal predicate. The proof that this was the binding defect
is extract v10's own profile — it trained to a *higher* reward than v8, 158.7
against 148.4, while losing the grip in 8,988 of 9,010 episodes.

`mdp.extraction_settling_penalty` charges residual module velocity over the last
60 mm of the pull, reads `EXTRACTION_LINEAR_VELOCITY_LIMIT` and
`EXTRACTION_ANGULAR_VELOCITY_LIMIT` rather than restating them, and is exactly
zero below them. 1,200 PPO epochs fine-tuned from extract v8 at 512 environments,
seed 70, one change. The precondition that killed the force-shaping work was
