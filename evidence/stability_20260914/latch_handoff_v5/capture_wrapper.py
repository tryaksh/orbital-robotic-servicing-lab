...
ROOT = Path('D:/portfolio/.lab-work/orbital-improvement/candidate')
sys.path.insert(0, str(ROOT / 'src'))
OUT = Path('D:/portfolio/.lab-work/orbital-improvement/smooth-fixed-v5')
...
if 'quintic' != 'legacy':
    argv.extend(['--transit_motion_profile', 'quintic'])
if 'guarded' != 'policy':
    argv.extend(['--extraction_finish', 'guarded'])
if True:
    argv.append('--transit_joint_trim')
if 'absolute_ik' != 'differential_ik':
    argv.extend(['--guarded_insert_solver', 'absolute_ik'])
...
hook = '''
        ...
        _portfolio_out = Path("D:/portfolio/.lab-work/orbital-improvement/smooth-fixed-v5")
        _portfolio_bodies = []
...
(OUT / 'capture_provenance.json').write_text(json.dumps({
    ...
    'source_commit': '88235d8c27314b6092b0aac38506af9526ceea40',
