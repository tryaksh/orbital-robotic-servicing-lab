...
| --- | ---: | --- | --- |
| `criterion_retention_v1.json` | That the quantity each criterion bounds still predicts the failure it names, read as a hand-over value ranking later outcomes | **The quantity is not a hand-over value.** `_freeze` in `run_workflow_demo.py` stores an episode's row *at the moment of judgement*, so the recorded grip error, attitude and velocity describe the state the outcome was decided in, not the state the episode entered with. For velocity the reading is circular by construction: `SEATED_CONDITIONS` includes `linear_velocity` and `angular_velocity`, so a failing episode fails partly because its velocity is high, which is why the scan returned an AUC of 1.000. The grip and attitude numbers are not circular but are concurrent associations, not predictions | `evidence/boundary_failure_modes_n192_v1.json` for the grip signature the same episodes support, and `evidence/boundary_lead_in_deletion_v1.json`, which is rate-based and unaffected |
